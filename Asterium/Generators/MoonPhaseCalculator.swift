...
struct MoonPhaseCalculator {
    ...
    static func calculate(for date: Date = Date()) -> MoonPhaseData {
        ...
        let displayIlluminationFraction = smoothedIlluminationFraction(
            from: rawIlluminationFraction
        )
        ...
        return MoonPhaseData(
            ...
            daysUntilNextPhase: daysUntilNextMajorPhase(from: moonAgeDays),
    ...
    static func getMoonAgeDays(for date: Date = Date()) -> Double {
        let adjustedDate = moonDayOffsetHours == 0 ? date : addHours(
            moonDayOffsetHours,
            to: date
        )
    ...
    private static func daysUntilNextMajorPhase(from moonAgeDays: Double) -> Int {
        let majorAges = [
            synodicMonth * 0.25,
            synodicMonth * 0.50,
            synodicMonth * 0.75,
        ...
        let nextAge = majorAges.first { $0 > moonAgeDays } ?? synodicMonth
        return max(Int(ceil(nextAge - moonAgeDays)), 0)
    }

    private static func energyDescription(
        phaseName: String,
        signName: String
    ) -> String {
        ...
        switch phaseName {
        case "New Moon":
            phaseEnergy = "Begin quietly and set clean intentions"
        case "Waxing Crescent":
            phaseEnergy = "Nurture early momentum and protect new plans"
        case "First Quarter":
            phaseEnergy = "Choose action over hesitation"
        case "Waxing Gibbous":
            phaseEnergy = "Refine what is already growing"
        case "Full Moon":
            phaseEnergy = "Let clarity, visibility, and release rise"
        case "Waning Gibbous":
            phaseEnergy = "Share wisdom and harvest what worked"
        case "Last Quarter":
            phaseEnergy = "Cut away friction and reset your rhythm"
        default:
            phaseEnergy = "Rest, cleanse, and make space"
        }

        return "\(phaseEnergy) through \(signName)'s lens."
    }
